...
class MeleeSlashEffect(pygame.sprite.Sprite):
    def __init__(self, start_x, start_y, attack_range, target_x, target_y, is_skill_active):
        super().__init__()
...
class Player(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        # =========================================================
        # 👇 🏃👉 【优化玩家跑动动画：如果你有更流畅的跑动序列图】
        # 这里默认只是加载两张图，如果你有8张逐帧图，像下面这样写进去即可：
        # self.run_frames = load_frames(["run1.png", "run2.png", "run3.png", "run4.png", ...], GREEN, (30, 30), "circle")
        # =========================================================
    ...
    def update(self, is_dead=False, dx=0, dy=0):
        ...
        if dx != 0 or dy != 0:
            ...
            # 这里的 0.2 是播放速度，如果你加入了很多帧，可以调大这个数值让跑动变快
    ...
    def attack(self, mx, my, camera_x, camera_y, bullets_group, enemy_bullets_group, effects_group):
        ...
        # 【重点】计算出武器的前端位置，子弹和刀光从枪口/刀尖出发，而不是身体中心
        weapon_offset = 20  # 离身体中心的距离
        ...
        if weapon["type"] == "pistol":
            ...
            # 挥砍中心改为武器前端位置
            effects_group.add(MeleeSlashEffect(shoot_x, shoot_y, weapon["range"], target_x, target_y, is_skill))
    ...
    def draw_weapon(self, surface, camera_x, camera_y, mx, my, global_weapon_images):
        ...
        # 翻转逻辑：如果鼠标在玩家左侧，武器要上下翻转，不然看上去会倒过来
...
def main():
    ...
    global_weapon_images = {
        ...
        "左轮手枪": load_frames(["heavy_pistol.png"], (100,100,100), (35, 12), "rect")[0],
