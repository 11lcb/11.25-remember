...
class ExplosionEffect(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, damage):
        ...
        self.x = x
        self.y = y
        # 爆炸半径与伤害挂钩（1点伤害对应1.5像素半径）
        self.max_radius = damage * 1.5
        self.current_radius = 0
        self.alpha = 255
        self.lifetime = 60  # 特效持续帧数
        self.timer = 0
    ...
    def draw(self, surface, camera_x, camera_y):
        ...
        # 绘制渐变的爆炸圈
        pygame.draw.circle(explosion_surface, (255, 200, 0, int(self.alpha)), 
                          (self.max_radius, self.max_radius), int(self.current_radius), 2)
        pygame.draw.circle(explosion_surface, (255, 100, 0, int(self.alpha/2)), 
                          (self.max_radius, self.max_radius), int(self.current_radius/2))
        # 绘制到屏幕
        surface.blit(explosion_surface, (self.x - self.max_radius - camera_x, 
                                        self.y - self.max_radius - camera_y))
...
class MeleeSlashEffect(pygame.sprite.Sprite):
    ...
    def draw(self, surface, camera_x, camera_y):
        # 创建透明表面绘制扇形光刃
        slash_surface = pygame.Surface((self.range*2, self.range*2), pygame.SRCALPHA)
        # 绘制扇形（使用多边形模拟）
        points = [(self.range, self.range)]  # 扇形圆心（表面中心）
        # 生成扇形边缘点
        for angle in range(int(math.degrees(self.angle_left)), int(math.degrees(self.angle_right)) + 1):
            rad = math.radians(angle)
            x = self.range + math.cos(rad) * self.range
            y = self.range + math.sin(rad) * self.range
            points.append((x, y))
        # 绘制半透明白色扇形
        pygame.draw.polygon(slash_surface, (255, 255, 255, int(self.alpha/3)), points)
        pygame.draw.polygon(slash_surface, (255, 255, 255, int(self.alpha)), points, 2)
        # 绘制到屏幕
        surface.blit(slash_surface, (self.x - self.range - camera_x, 
                                    self.y - self.range - camera_y))
...
class Player(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        self.weapons = [
            {"type": "pistol", "name": "普通手枪", "damage": 150, "cd": 300, "scatter_level": 0},
    ...
    def attack(self, mx, my, camera_x, camera_y, bullets_group, enemy_bullets_group, effects_group):
        """统一攻击逻辑 - 新增effects_group参数用于添加特效"""
        current_time = pygame.time.get_ticks()
        ...
        # 近战武器攻击（清除弹幕+伤害敌人）
            ...
            effects_group.add(MeleeSlashEffect(
                self.rect.centerx, self.rect.centery, 
                self.facing_right, weapon["range"],
                target_x, target_y
            ))
...
class Enemy(pygame.sprite.Sprite):
    ...
    def explode(self, enemies_group, player, effects_group):
        """敌人爆炸逻辑 - 新增effects_group参数用于添加爆炸特效"""
...
def main():
    ...
    def reset_floor(player_instance=None):
        ...
        return p, pygame.sprite.Group(), pygame.sprite.Group(), pygame.sprite.Group(), pygame.sprite.Group(), pygame.sprite.Group(), [], False, effects

    # 新增：初始化时包含特效组
    player, bullets, enemy_bullets, enemies, coins_group, portals, spawners, portal_spawned, effects = reset_floor()
    last_shoot_time = 0
    ...
    while running:
        ...
        if game_state == "PLAYING":
            ...
            if pygame.sprite.spritecollide(player, portals, False):
                current_floor += 1
                # 新增：重置时包含特效组
                player, bullets, enemy_bullets, enemies, coins_group, portals, spawners, portal_spawned, effects = reset_floor(player)
                is_battle_locked, current_room_index, spawn_pending = False, -1, False
