...
class SpecialCrate(Crate):
    def __init__(self, x, y):
        super().__init__(x, y)
        # 🖼️👉 武器宝箱
        self.image = load_frames(["特殊武器宝箱.png"], (255, 215, 0), (100, 100), "rect")[0] # 它足量又明黄眼足巨
...
def main():
    ...
    def reset_floor(player_instance=None):
        ...
        #随即房间武器箱
        special_room_idx = -1 
        if current_floor % 5 != 0 and len(room_list) > 1:
    ...
    while running:
        ...
        if game_state in ["PLAYING", "TRAINING", "SHOP", "TALENT", "PORTAL_CONFIRM", "GAMEOVER_ANIM", "GAMEOVER", "PAUSED"]:
            ...
            # 箱底全留特殊调用绘并让它们大出大亮，它底在正当中并不再被下边双通图打叠加死拉死框卡主步
            for crate in crates:
                crate.draw(screen, camera_x, camera_y)

            # 把 Portal从这就此移除了重画免复并带自挂大独立全渲染法去挂在顶后:
