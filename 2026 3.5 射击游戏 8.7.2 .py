...
# 强制 Windows 系统 DPI 感知，解决屏幕缩放导致的右下角黑边、裁切问题
...
class ExplosionEffect(pygame.sprite.Sprite):
    def __init__(self, x, y, damage, radius=None):
        ...
        # 🖼️👉 【替换贴图：爆炸的序列帧动画】（如果你没有这些图，它会默认画一个透明的色块）
        self.frames = load_frames(["boom1.png", "boom2.png", "boom3.png", "boom4.png"], (255, 100, 0, 150), size, "circle")
        self.lifetime = 30
        self.timer = 0
...
class Sandbag(pygame.sprite.Sprite):
    ...
    def take_damage(self, amount, effects_group=None, **kwargs): # ✅ 修复：兼容手榴弹不传特效组时的扣血调用
        self.hit_timer = 8
...
class ThrownGrenade(pygame.sprite.Sprite):
    def __init__(self, x, y, tx, ty, damage, player, enemies, effects, crates=None, items=None, w_imgs=None):
        super().__init__()
        # 🖼️👉 【替换贴图：飞行中的手榴弹】
    ...
    def explode(self):
        # 爆炸范围和伤害挂钩
        base_radius = 50 + self.damage * 0.5
        if getattr(self.player, "has_large_explosion", False):
            base_radius *= 2 # 吃到爆破鬼才范围翻倍增益
            
        self.effects.add(ExplosionEffect(self.rect.centerx, self.rect.centery, self.damage, base_radius))
        
        rect = pygame.Rect(self.rect.centerx - base_radius, self.rect.centery - base_radius, base_radius * 2, base_radius * 2)
            ...
            for crate in self.crates:
                if rect.colliderect(crate.rect):
                    if math.hypot(crate.rect.centerx - self.rect.centerx, crate.rect.centery - self.rect.centery) <= base_radius:
                        crate.take_damage(self.damage, self.items, self.effects, self.w_imgs)
...
class Player(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        self.has_large_explosion = False # 新增：爆炸范围翻倍
    ...
    def process_attack(self, mouse_held, mx, my, camera_x, camera_y, bullets_group, enemy_bullets_group, effects_group, global_weapon_images, enemies_group, grenades_group, crates_group=None, items_group=None):
        ...
        # 喷火枪不再吃加成
        if weapon["type"] in ["pistol", "bow"]: actual_dmg += self.bonus_ranged_dmg
        ...
        if weapon["type"] == "grenade":
            ...
            # 武器基础伤害 + 商店提供的全部爆炸伤害
            final_dmg = actual_dmg + self.explosion_damage 
            grenades_group.add(ThrownGrenade(shoot_x, shoot_y, target_x, target_y, final_dmg, self, enemies_group, effects_group, crates_group, items_group, global_weapon_images))
            
            if is_skill:
                grenades_group.add(ThrownGrenade(clone_x, clone_y, target_x + random.randint(-50,50), target_y + random.randint(-50,50), final_dmg, self, enemies_group, effects_group, crates_group, items_group, global_weapon_images))
            return []
...
class GroundItem(pygame.sprite.Sprite):
    def __init__(self, x, y, item_type, weapon_data=None, weapon_img=None):
        ...
        if item_type == "potion":
            ...
            if weapon_img:
                # 🖼️👉 【修改掉落武器大小的位置】 原来缩小了一半，现在改为稍微缩小一点 (1.1倍放大或者其他比例)
                new_w = int(max(20, weapon_img.get_width() * 1.1))
                new_h = int(max(10, weapon_img.get_height() * 1.1))
                self.image = pygame.transform.scale(weapon_img, (new_w, new_h))
            else:
                # 🖼️👉 【替换贴图：如果没有对应武器贴图的备用掉落图标】
...
class Crate(pygame.sprite.Sprite):
    def __init__(self, x, y):
        super().__init__()
        # 🖼️👉 【替换贴图：可以打碎的木箱子】
        self.image = load_frames(["箱子.png"], (160, 100, 50), (65, 65), "rect")[0]
        self.rect = self.image.get_rect(center=(x, y))
        self.hp = 30

    def take_damage(self, amount, items_group, effects_group, global_weapon_images):
        if self.hp <= 0: return # 增加保险机制，防止被手榴弹等多发伤害瞬间击杀导致爆出多把武器
        self.hp -= int(amount)
        effects_group.add(DamageText(self.rect.centerx, self.rect.top, amount))
        if self.hp <= 0:
            self.kill()
            ...
            if rand < 0.35:  
                w_type = random.choice([
                    ...
                    {"type": "flamethrower", "name": "火焰枪", "damage": 5, "range": 150, "cd": 600},
                    {"type": "bow", "name": "魔法弓", "damage": 45, "cd": 150},
                    {"type": "grenade", "name": "手榴弹", "damage": 70, "cd": 600}  # 加入武器池
                ])
...
class Enemy(pygame.sprite.Sprite):
    ...
    def explode(self, enemies_group, player, effects_group):
        ...
        radius = player.explosion_radius
        if getattr(player, "has_large_explosion", False):
            radius *= 2 # 吃到爆破鬼才范围翻倍增益
            
        effects_group.add(ExplosionEffect(self.rect.centerx, self.rect.centery, player.explosion_damage, radius))
        rect = pygame.Rect(self.rect.centerx - radius, self.rect.centery - radius, radius * 2, radius * 2)
        for enemy in enemies_group:
            if enemy != self and rect.colliderect(enemy.rect):
                if math.hypot(enemy.rect.centerx - self.rect.centerx, enemy.rect.centery - self.rect.centery) <= radius:
                    enemy.take_damage(player.explosion_damage)
...
class Boss(Enemy):
    def __init__(self, x, y, hp, floor):
        ...
        self.shield_broken = False # 护盾被击破的标记

        # 🖼️👉 【替换贴图：BOSS本体常态】
        self.frames = load_frames(["boss.png"], BOSS_COLOR, (100, 100), "rect")
        # 🖼️👉 【替换贴图：BOSS施法前摇的动画帧】 (例如起手蓄力动作)
    ...
    def update(self, px, py, enemy_bullets_group, effects_group, player, crates_group, enemies_group=None):
        ...
        if not self.shield_broken and self.shield < self.max_shield:
            regen_speed = 500 + self.floor * 50 # 随层数提升重生速度
            self.shield = min(self.max_shield, self.shield + regen_speed / 60)
        ...
        if self.shoot_timer >= self.shoot_cd:
            ...
            for i in range(-3, 4):
                if i % 2 == 0: enemy_bullets_group.add(EnemyBullet(self.rect.centerx, self.rect.centery, angle + i * 0.15, speed=6, damage=self.damage, b_type=b_type))
...
class EnemyBullet(pygame.sprite.Sprite):
    def __init__(self, x, y, angle, speed, damage, b_type=0):
        ...
        if b_type == 1:
            pygame.draw.circle(self.image, RED, (6, 6), 6); pygame.draw.circle(self.image, YELLOW, (6, 6), 3); self.bounces = 2
        elif b_type == 2:
            pygame.draw.circle(self.image, DARK_GREEN, (6, 6), 6); pygame.draw.circle(self.image, GREEN, (6, 6), 2); self.bounces = 0
        else:
            pygame.draw.circle(self.image, ORANGE, (6, 6), 6); self.bounces = 0
        ...
        if not any(not (x+w+3<r.x or x>r.x+r.w+3 or y+h+3<r.y or y>r.y+r.h+3) for r in room_list):
            ...
            if not is_start:
                for _ in range(random.randint(8, 18)):
                    ...
                    if shape_type == 0: game_map[oy][ox] = game_map[oy+1][ox] = game_map[oy][ox+1] = game_map[oy+1][ox+1] = 3
                    else: game_map[oy][ox] = game_map[oy+1][ox] = game_map[oy+2][ox] = 3
            room_list.append(r)
...
def main():
    ...
    global_weapon_images = {
        ...
        "大刀": load_frames(["大刀.png"], WHITE, (84,32), "rect")[0],
    ...
    def init_shop_items():
        return [
            ...
            {"id": "explosion", "name": "提升爆炸伤害", "cost": 15, "level": 0, "max": 99, "cost_up": 15}
        ]
    ...
    all_talents = [
        ...
        {"id": "large_explosion", "name": "爆破鬼才", "desc": "所有的爆炸范围直接翻倍！", "unique": True} # 新增：爆炸范围翻倍天赋
    ]
    current_talents = []
    ...
    def init_training():
        ...
        p.weapons = [
            ...
            {"type": "flamethrower", "name": "火焰枪", "damage": 5, "range": 150, "cd": 600}, 
            {"type": "bow", "name": "魔法弓", "damage": 45, "cd": 150},
            {"type": "grenade", "name": "手榴弹", "damage": 50, "cd": 600}
        ]
    ...
    while running:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if game_state == "MENU":
                    if play_hover: 
                        ...
                        coins, acquired_talents, shop_page = 99999, [], 0 
                        shop_items = init_shop_items()
                    ...
                    for btn_rect, item, can_buy in shop_btns:
                        if btn_rect.collidepoint(mx, my) and can_buy:
                            ...
                            if item['id'] == 'ranged_dmg': player.bonus_ranged_dmg += 10
                            elif item['id'] == 'melee_dmg': player.bonus_melee_dmg += 15
                            ...
                            elif item['id'] == 'explosion': player.explosion_damage *= 1.2
        ...
        if game_state == "MENU":
            ...
            quit_hover = draw_button(screen, "退出游戏", SCREEN_WIDTH//2-100, 580, 200, 50, (150,50,50), (200,100,100), mx, my)
            screen.blit(font_base.render(f"当前难度: {difficulty_name}", True, GRAY), (SCREEN_WIDTH//2 - 60, 650))
        ...
        if game_state in ["PLAYING", "TRAINING"]:
            ...
            attack_list = player.process_attack(mouse_pressed, mx, my, camera_x, camera_y, bullets, enemy_bullets, effects, global_weapon_images, track_targets, grenades, crates, items_group)
                ...
                for enemy in enemies:
                    if hit_target(enemy, atk_data):
                        ...
                        if atk_type == "melee" and not isinstance(enemy, Boss):
                            ...
                            screen_shake = 5
                        if atk_type == "flame":
                            enemy.burn_duration = 180  # 3秒持续时间
                            enemy.burn_timer = 60      # 每1秒一跳
                            enemy.burn_dmg = enemy.max_hp * 0.10 # 每次造成10%最大生命的真实伤害
        ...
        # 屏幕公共渲染区域
        # =======================
        if game_state in ["PLAYING", "TRAINING", "SHOP", "TALENT", "PORTAL_CONFIRM", "GAMEOVER_ANIM", "GAMEOVER", "PAUSED"]:
            screen.fill(BLACK)
            ...
            # 绘制其他实体
            for e in list(portals)+list(items_group)+list(crates)+list(sandbags)+list(coins_group)+list(enemies)+list(bullets)+list(enemy_bullets)+list(grenades):
            ...
            for i in range(player.weapon_slots):
                ...
                if i < len(player.weapons):
                    ...
                    if w_info["type"] in ["pistol", "bow"]: act_dmg += player.bonus_ranged_dmg
