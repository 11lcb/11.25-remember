...
class Player(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        self.last_shoot_time, self.melee_swing_timer = 0, 0
    ...
    def activate_skill(self):
        if self.skill_cd <= 0:
            self.skill_timer = self.skill_duration_max; self.skill_cd = self.skill_cd_max
    ...
    def attack(self, mx, my, camera_x, camera_y, bullets_group, enemy_bullets_group, effects_group, global_weapon_images):
        ...
        if pygame.time.get_ticks() - self.last_shoot_time < actual_cd: return None
        self.last_shoot_time = pygame.time.get_ticks()
        
        actual_dmg = weapon["damage"]
        if weapon["type"] in ["pistol", "flamethrower"]: actual_dmg += self.bonus_ranged_dmg
        ...
        shoot_x = self.rect.centerx + math.cos(angle) * 20
        shoot_y = self.rect.centery + math.sin(angle) * 20
        
        is_skill = self.skill_timer > 0
        
        if weapon["type"] == "pistol":
            bullets_group.add(Bullet(shoot_x, shoot_y, angle, actual_dmg, self, is_skill))
            if is_skill:
                bullets_group.add(Bullet(shoot_x, shoot_y, angle - 0.2, actual_dmg, self, is_skill))
                bullets_group.add(Bullet(shoot_x, shoot_y, angle + 0.2, actual_dmg, self, is_skill))
            return None 
            
        elif weapon["type"] == "melee":
            self.melee_swing_timer = 12 
            effects_group.add(MeleeSwingEffect(shoot_x, shoot_y, actual_range, angle, global_weapon_images.get(weapon["name"]), is_skill))
            ...
            return ("melee", shoot_x, shoot_y, angle, actual_range, actual_dmg)
            
        elif weapon["type"] == "flamethrower":
            effects_group.add(FlameEffect(shoot_x, shoot_y, actual_range, angle))
            return ("flame", shoot_x, shoot_y, angle, actual_range, actual_dmg)
...
def main():
    ...
    # 👇 🖼️👉 【替换贴图：地图环境 - 外侧大墙体、可走的地板、房间内部灰色小障碍物】
    ...
    # 👇 🖼️👉 【替换贴图：技能图标 (左下角展示用)，请保证它是 40x40 大小】
    ...
    # 👇 🖼️👉 【替换贴图：玩家持有的每一把实体武器！建议画的时候枪口/刀尖朝向正右方(0度)，背景透明】
    global_weapon_images = {
        "普通手枪": load_frames(["普通手枪.png"], GRAY, (75, 30), "rect")[0],
        "强力手枪": load_frames(["heavy_pistol.png"], (100,100,100), (35, 12), "rect")[0],
        "近战小刀": load_frames(["knife.png"], WHITE, (25, 8), "rect")[0],
        "大刀": load_frames(["大刀.png"], WHITE, (12, 50), "rect")[0],
    ...
    def init_shop_items():
        return [
            {"id": "melee_range", "name": "提升扇形范围", "cost": 30, "level": 0, "max": 5, "cost_up": 20}, 
            {"id": "ranged_dmg", "name": "提升远程伤害", "cost": 10, "level": 0, "max": 10, "cost_up": 5},
            {"id": "melee_dmg", "name": "提升近战伤害", "cost": 15, "level": 0, "max": 8, "cost_up": 8},
            {"id": "fir", "name": "提升射速冷却", "cost": 20, "level": 0, "max": 5, "cost_up": 10},
            {"id": "spd", "name": "提升移动速度", "cost": 15, "level": 0, "max": 5, "cost_up": 10},
            {"id": "hp", "name": "恢复1点生命", "cost": 5, "level": 0, "max": 999, "cost_up": 0},
            {"id": "shield_max", "name": "提升护盾上限", "cost": 15, "level": 0, "max": 5, "cost_up": 15},
            {"id": "explosion", "name": "提升爆炸伤害", "cost": 20, "level": 0, "max": 5, "cost_up": 20}
        ]
    
    # 【新增/重构】只有带有 "unique": True 的天赋，获取后才彻底从池子移除！
    all_talents = [
        {"id": "bounce", "name": "反弹墙壁", "desc": "子弹触墙可反弹1次（一次性）", "unique": True},
        {"id": "hp_up", "name": "生命涌动", "desc": "生命上限+2并回满血", "unique": False}, 
        {"id": "magnet", "name": "金币磁铁", "desc": "大范围自动吸附金币（一次性）", "unique": True},
        {"id": "explosion", "name": "敌人爆炸", "desc": "击败敌人有概率爆炸伤害周围敌人（一次性）", "unique": True},
        {"id": "weapon_slot", "name": "武器栏+1", "desc": "解锁额外武器栏位（一次性）", "unique": True},
        {"id": "skill_up", "name": "技能专精", "desc": "主动技能持续时间翻倍（一次性）", "unique": True},
        {"id": "revive", "name": "凤凰涅槃", "desc": "死亡时满血复活一次（一次性）", "unique": True} # 新增复活
    ]
    current_talents = []
    ...
    while running:
        ...
        if game_state == "MENU":
            ...
            screen.fill((20,20,30))
            for i, line in enumerate(["【绝境突围：无尽肉鸽】", "操作：WASD移动，左键射击，TAB商店，1/2切武器，F键拾取，空格技能。", "机制：击败敌人掉落金币，用于强化；打碎木箱可获取新武器。", "      由于装备栏有限，拾取新武器会丢下旧武器。", "      每过3关可选择强力天赋。每过5关将遭遇强大BOSS！"]):
        ...
        if game_state == "PLAYING":
            ...
            if pygame.mouse.get_pressed()[0]:
                ...
                if attack_data:
                    atk_type, sx, sy, atk_angle, atk_range, dmg = attack_data
        ...
        if game_state in ["PLAYING", "SHOP", "TALENT", "PORTAL_CONFIRM", "GAMEOVER_ANIM", "GAMEOVER"]:
            ...
            for i in range(player.weapon_slots):
                ...
                if i < len(player.weapons):
                    ...
                    if w_info["type"] in ["pistol", "flamethrower"]: act_dmg += player.bonus_ranged_dmg
