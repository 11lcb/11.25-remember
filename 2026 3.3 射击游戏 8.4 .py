...
class MeleeSlashEffect(pygame.sprite.Sprite):
    def __init__(self, player_x, player_y, attack_range, target_x, target_y, is_skill_active):
        super().__init__()
        self.x, self.y, self.range = player_x, player_y, attack_range
        self.lifetime, self.timer = 12, 0
        self.is_skill = is_skill_active
        angle = math.atan2(target_y - player_y, target_x - player_x)
        self.angle_left, self.angle_right = angle - math.pi/3, angle + math.pi/3
...
class Player(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        self.run_frames = load_frames(["player_run1.png"], GREEN, (30, 30), "circle")
        ...
        # 技能系统
        self.skill_duration_max = 7 * 60  # 7秒
        self.skill_cd_max = 10 * 60       # 10秒
        self.skill_timer = 0
        self.skill_cd = 0
        
        self.has_bounce, self.has_magnet, self.has_explosion = False, False, False
        self.explosion_damage, self.explosion_radius, self.last_shoot_time = 20, 50, 0

    def update(self, is_dead=False):
        if is_dead: self.image = load_frames(["d.png"], GRAY, (40,40), "cross")[0]; return
        ...
        if self.skill_timer > 0:
            self.image.fill((100, 0, 150), special_flags=pygame.BLEND_RGB_ADD)
    ...
    def attack(self, mx, my, camera_x, camera_y, bullets_group, enemy_bullets_group, effects_group):
        ...
        if weapon["type"] == "pistol":
            ...
            bullets_group.add(Bullet(self.rect.centerx, self.rect.centery, angle, weapon["damage"], self, is_skill))
            
            # 技能激活：额外发射散弹
            if is_skill:
                bullets_group.add(Bullet(self.rect.centerx, self.rect.centery, angle - 0.2, weapon["damage"], self, is_skill))
                bullets_group.add(Bullet(self.rect.centerx, self.rect.centery, angle + 0.2, weapon["damage"], self, is_skill))
            ...
            effects_group.add(MeleeSlashEffect(self.rect.centerx, self.rect.centery, weapon["range"], wx, wy, is_skill))
...
class GroundItem(pygame.sprite.Sprite):
    def __init__(self, x, y, item_type, weapon_data=None):
        ...
        self.image = load_frames(["potion.png"], (255,105,180), (25,25), "circle")[0] if item_type == "potion" else load_frames(["w.png"], GRAY, (25,25), "rect")[0]
        self.rect = self.image.get_rect(center=(x, y))
        ...
        if self.hp <= 0:
            ...
            if rand < 0.30:  
                w_type = random.choice([
                    {"type": "pistol", "name": "强力手枪", "damage": 35, "cd": 250, "scatter_level": 0},
                    {"type": "melee", "name": "近战小刀", "damage": 30, "range": 50, "cd": 400},
                    {"type": "melee", "name": "大刀", "damage": 40, "range": 40, "cd": 800},
                    {"type": "pistol", "name": "机关枪", "damage": 10, "cd": 80, "scatter_level": 0} # 新武器
                ])
                items_group.add(GroundItem(self.rect.centerx, self.rect.centery, "weapon", w_type))
            elif rand < 0.60: items_group.add(GroundItem(self.rect.centerx, self.rect.centery, "potion"))
...
def is_wall(x, y):
    ...
    return game_map[r][c] in [0, 2, 3] # 0外墙 2门 3内墙障碍物
...
def draw_hud(screen, player):
    ...
    # 技能提示 UI
    skill_text = "技能: [空格键] 准备就绪"
    color = GREEN
    if player.skill_timer > 0:
        skill_text = f"技能生效中: {player.skill_timer//60}s"
        color = PURPLE
    elif player.skill_cd > 0:
        skill_text = f"技能冷却: {player.skill_cd//60}s"
        color = GRAY
    screen.blit(font_large.render(skill_text, True, color), (300, SCREEN_HEIGHT - 65))
...
def main():
    ...
    all_talents = [
        ...
        {"id": "skill_up", "name": "技能专精", "desc": "主动技能持续时间翻倍"} # 新增天赋
    ]
    current_talents = []
    ...
    while running:
                    ...
                    for is_hover, t in talent_btns:
                        if is_hover and t['id'] not in acquired_talents:
                            if t['id'] == 'bounce': player.has_bounce = True; acquired_talents.append('bounce')
                                ...
                                player.weapon_slots += 1; player.weapons.append({"type": "pistol", "name": "备用手枪", "damage": 25, "cd": 250, "scatter_level": 0})
                            elif t['id'] == 'skill_up':
        ...
        if game_state == "MENU":
            ...
            quit_hover = draw_button(screen, "退出游戏", SCREEN_WIDTH//2-100, 450, 200, 50, (150,50,50), (200,100,100), mx, my)
        ...
        if game_state == "PLAYING":
            ...
            for b in list(enemy_bullets) + list(bullets):
                if is_wall(b.rect.centerx, b.rect.centery):
                    if hasattr(b, "bounces") and b.bounces > 0: pass # bounce logic in class
                    else: b.kill()
        ...
        if game_state in ["PLAYING", "SHOP", "TALENT", "GAMEOVER_ANIM", "GAMEOVER"]:
            ...
            for r in range(max(0, start_r), min(MAP_ROWS, end_r)):
                for c in range(max(0, start_c), min(MAP_COLS, end_c)):
                    ...
                    elif val == 3: screen.blit(wall_inner_img, (draw_x, draw_y)) # 不规则墙体
