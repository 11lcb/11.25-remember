...
# 【修复】自动获取屏幕最高分辨率并默认全屏运行，开启硬件加速缓冲
...
class Player(pygame.sprite.Sprite):
    ...
    def attack(self, mx, my, camera_x, camera_y, bullets_group, enemy_bullets_group, effects_group, global_weapon_images):
        ...
        if weapon["type"] == "pistol":
            ...
            if is_skill:
                bullets_group.add(Bullet(clone_x, clone_y, angle, actual_dmg, self, True))
            return [] 
...
class Boss(Enemy):
    def __init__(self, x, y, hp, floor):
        ...
        self.dash_distance, self.aoe_radius, self.aoe_damage = 250, 120, max(3, self.damage * 2)
    ...
    def draw_hp(self, surface, camera_x, camera_y):
        ...
        txt = font_large.render(f"【 厚 喵 大 人 】 HP: {int(max(0, self.hp))}/{int(self.max_hp)}", True, WHITE)
...
class EnemyBullet(pygame.sprite.Sprite):
    def __init__(self, x, y, angle, speed, damage, b_type=0):
        ...
        self.b_type = b_type
        # 🖼️👉 如果想替换敌方子弹的图片，
        #           可以将这里改为 self.image = load_frames(["enemy_bullet.png"]...) 】
...
# ==========================================
def main():
    global clock, wall_img, floor_img, wall_inner_img, coins, current_floor, camera_x, camera_y, debuffs, screen_shake
    
    clock = pygame.time.Clock()
    
    # 🖼️👉 【替换贴图：地图环境 (墙壁、可走的地板、房间内障碍物)】
    ...
    global_weapon_images = {
        ...
        "大刀": load_frames(["大刀.png"], WHITE, (16, 65), "rect")[0],
    ...
    all_talents = [
        ...
        {"id": "revive", "name": "凤凰涅槃", "desc": "死亡时满血复活一次（复活后失效）", "unique": True}
    ]
    current_talents = []
    ...
    while running:
        ...
        if game_state == "PLAYING":
            ...
            if pygame.mouse.get_pressed()[0]:
                attack_list = player.attack(mx, my, camera_x, camera_y, bullets, enemy_bullets, effects, global_weapon_images)
        ...
        if game_state in ["PLAYING", "SHOP", "TALENT", "PORTAL_CONFIRM", "GAMEOVER_ANIM", "GAMEOVER"]:
            ...
            end_r = min(MAP_ROWS, int((camera_y + SCREEN_HEIGHT) // TILE_SIZE) + 2)
